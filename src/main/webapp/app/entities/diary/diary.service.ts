...
export class DiaryService {
    ...
    create(diary: IDiary): Observable<EntityResponseType> {
        return this.http.post<IDiary>(this.resourceUrl, diary, { observe: 'response' });
    }

    update(diary: IDiary): Observable<EntityResponseType> {
        return this.http.put<IDiary>(this.resourceUrl, diary, { observe: 'response' });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<IDiary>(`${this.resourceUrl}/${id}`, { observe: 'response' });
    }

    query(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IDiary[]>(this.resourceUrl, { params: options, observe: 'response' });
    }
